...
export const updateNavLinks = (mode: Mode) => {
  ...
  if (isCyber) {
    ...
    if (cTitle) cTitle.setAttribute("data-text", "CYBER SECURITY");
    if (cTitle) cTitle.textContent = "CYBER SECURITY";
  } else {
    navHeaders.forEach((el, i) => {
    ...
    if (bgTitle) bgTitle.textContent = "My Background";
    const servTitle = document.querySelector("#service .title");
    if (servTitle) servTitle.textContent = "Frontend Development";
    const projTitle = document.querySelector("#project .title");
    if (projTitle) projTitle.textContent = "Projects";
    const contTitle = document.querySelector("#contact .title");
    if (contTitle) contTitle.textContent = "Contact Me";
  }
};

export const updateModeIcon = (mode: Mode) => {
  if (!cyberBtn) return;
  if (mode === "cyber") {
    // Brush Icon
...
export const applyMode = (mode: Mode) => {
  if (mode === "cyber") {
    ...
    const subtitle = document.querySelector(".cyber-subtitle") as HTMLElement;
    if (subtitle) {
